...
Claude Code 대화를 자동 로깅하는 플러그인 (v0.2.2). Python 3.6+ (stdlib only), Claude Code v1.0.33+.
...
```
UserPromptSubmit 훅 (10s)          Stop 훅 (30s)
    │                              │
    ▼                              ▼
...
- `scripts/` — 핵심 로직 (3개 모듈). stdlib만 허용, 외부 패키지 import 금지
...
- 분류 함수: `classify_user_entry()`가 반환하는 타입 문자열로 처리 분기 (e.g., `"TOOL_REJECTION"`, `"PLAN_APPROVAL"`)
